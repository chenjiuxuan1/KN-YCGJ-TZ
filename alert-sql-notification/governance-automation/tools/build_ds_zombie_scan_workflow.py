...
ARGS=(--country {code} --batch-id '{{$json.batch_id}}' --lookback-days '{{$json.lookback_days}}' --min-stale-months '{{$json.min_stale_months}}' --score-version '{{$json.score_version}}' --top-limit '{{$json.top_limit}}' --release-state '{{$json.release_state}}')
...
def build_workflow():
    nodes = [
        ...
        {"parameters": {"jsCode": """const c={country:'th',minStaleMonths:3,lookbackDays:30,releaseState:'online',projectName:'',workflowName:'',taskName:'',writeToDb:false,dryRun:true,scoreVersion:'v1',topLimit:0}; if(!['cn','ph','ine','mx','th','pk'].includes(c.country)) throw new Error('country 必须为 cn、ph、ine、mx、th 或 pk'); if(!['all','online','offline'].includes(c.releaseState)) throw new Error('release_state 必须为 all、online 或 offline'); const id=String(Date.now()); return [{json:{request_id:id,batch_id:id,country:c.country,min_stale_months:c.minStaleMonths,lookback_days:c.lookbackDays,release_state:c.releaseState,project_name:c.projectName,workflow_name:c.workflowName,task_name:c.taskName,write_to_db:c.writeToDb,dry_run:c.dryRun,score_version:c.scoreVersion,top_limit:Math.max(0,Number(c.topLimit)||0)}}];"""}, "id": "request", "name": "Build Manual Scan Request", "type": "n8n-nodes-base.code", "typeVersion": 2, "position": [-680, 300]},
