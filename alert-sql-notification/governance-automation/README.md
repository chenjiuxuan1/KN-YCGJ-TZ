...
`tools/build_ds_match_candidate_workflow.py` 会生成两个 n8n 子流程：

- `DS任务匹配代码部署.json`: 手动执行，用于在各国 SSH 节点机器拉取或更新 GitHub 代码。
- `DS任务匹配候选查询_execute_workflow.json`: 被主通知流程调用，只执行已部署的 Python 脚本，不再每次运行都更新代码。
...
1. 导入并手动执行 `DS任务匹配代码部署`，确认各国 SSH 节点机器已拉取代码。
2. 导入 `DS任务匹配候选查询_execute_workflow`。
3. 导入主通知增强版 workflow，并在主流程里选择该子 workflow。
4. 后续只有代码更新时才重新执行 `DS任务匹配代码部署`；普通异常 SQL 通知运行时不会执行 git 更新。
